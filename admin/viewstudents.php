...
if (isset($_GET['submit'])) {
  // Check if 'searchYear' is set
  if (isset($_GET['searchYear'])) {
      $searchYear = mysqli_real_escape_string($conn, $_GET['searchYear']);
      // Initialize $branch as an empty string
      $branch = '';

      // Check if 'branch' is set and is an array
      if (isset($_GET['branch']) && is_array($_GET['branch'])) {
          // Escape each element in the array
          $escapedBranches = array_map(function ($branch) use ($conn) {
              return mysqli_real_escape_string($conn, $branch);
          }, $_GET['branch']);

          // Combine the escaped branches into a string
          $branch = implode("','", $escapedBranches);
      }

      // Modify the SQL query to include both search conditions
      $sql = "SELECT * FROM studentlogin";

      // Check if either branch or searchYear is present to add WHERE clause
      if (!empty($branch) || !empty($searchYear)) {
          $sql .= " WHERE";

          // Add branch condition if present
          if (!empty($branch)) {
              $sql .= " branch IN ('$branch')";
          }

          // Add AND keyword if both branch and searchYear conditions are present
          if (!empty($branch) && !empty($searchYear)) {
              $sql .= " AND";
          }

          // Add searchYear condition if present
          if (!empty($searchYear)) {
              $sql .= " yop = '$searchYear'";
          }
      }
      ...
      // If searchYear is not set, retrieve all data
      $sql = "SELECT * FROM studentlogin ORDER BY time DESC; ";
  }
} else {
...
?>  
    ...
    #placementTable tr:hover {
        ...
        .feature-box {
            ...
            box-shadow: 0px 4px 8px rgba(0, 0, 0, 0.1), 0px 4px 16px rgba(0, 0, 0, 0.1);
            margin: 10px;
        }
        ...
        .navy-button {
            ...
            width: 200px; /* Set width for all buttons */
                    ...
                    <ul class="navMenu">
                                ...
                                class="active" title="Student">
                                                        ...
                                                        <ul class="dropMenu">
                                                                                                                                ...
                                                                                                                                <li><a href="addstudents.php" title="Add Student">
                                                                                Add Student
                                        </a></li>

                                        <li><a href="viewstudents.php" class="active" title="View Student">
                                                                                View Student
                                        </a></li>
...
<h1 class="form-row justify-content-center" style=" 
            text-align: center;
    ...
    <form method="GET" action="">
      <label for="searchYear">Search by Year of Passing:</label>
      <input type="text" name="searchYear" id="searchYear" placeholder="Enter Year" value="<?php echo $searchYear; ?>">
      <br><br>
      <label>Select Branch(es):</label>
      <label><input type="checkbox" name="branch[]" value="CST" <?php echo (in_array('CST', (array)$branch) ? 'checked' : ''); ?>> CST</label>
      <label><input type="checkbox" name="branch[]" value="IT" <?php echo (in_array('IT', (array)$branch) ? 'checked' : ''); ?>> IT</label>
       <label><input type="checkbox" name="branch[]" value="CE" <?php echo (in_array('CE', (array)$branch) ? 'checked' : ''); ?>> CE</label>
      <label><input type="checkbox" name="branch[]" value="ENC" <?php echo (in_array('ENC', (array)$branch) ? 'checked' : ''); ?>> ENC</label>
      <label><input type="checkbox" name="branch[]" value="DS" <?php echo (in_array('DS', (array)$branch) ? 'checked' : ''); ?>> DS</label>
      <label><input type="checkbox" name="branch[]" value="AI" <?php echo (in_array('AI', (array)$branch) ? 'checked' : ''); ?>> AI</label>
    </br></br>
    <button type="submit" name="submit">Search</button>
  </div>
    </form>
    
    <div class="container" style="width: 200%; margin-left: 5px;">
    ...
    <form>
      ...
      <table class="table table-hover table-borderless table-dark">
            ...
            <tr>
              ...
              <th scope="col">B.tech Percentage</th>
              <th scope="col">SSC</th>
              <th scope="col">HSC</th>
     ...
     $seq = 1;
              ...
              ?>
                  ...
                  <td>
                    <a href="editstud.php?edit=<?php echo $row['id']; ?>" class="btn btn-sm" name="deleteloc"><i class="fas fa-pen" style="color: #3498DB;"></i></a>
                    <a href="php/crud.php?delete4=<?php echo $row['id']; ?>" class="btn btn-sm" name="deleteloc"><i class="fas fa-trash" style="color: red;"></i></a>
                  </td>
    ...
    <div id="export-section">
        <a href="export_students.php"><button class="btn btn-sm btn-primary">Export to CSV <i class="glyphicon glyphicon-export"></i></button></a>
    </div>
  </div>
</main>
                        ...
                        <div class="col-lg-2">
                            <div class="footer-box">
                                                                ...
                                                                <ul>
                                                                        ...
                                                                        <li><a href="../feedback" target="_blank" title="Feedback / Queries"> Feedback / Queries</a>
                                    </li>
                                                                                                                                            </ul>
                            </div>
                        </div>

...
<body>
    ...
    <div class="row">
        <div class="col-md-6 offset-md-3 mt-4">
            <form action="applycompany.php" method="POST">
                <div class="form-group">
                    <label for="exampleInputJobType">Select Job Type</label>
                    <select class="custom-select" name="jobtype">
                        <?php
                        // Assuming you have already established a database connection
                        // Fetch distinct job types from the database
                        $sql_job_types = "SELECT DISTINCT jobtype FROM job WHERE status='active';";
                        $result_job_types = mysqli_query($conn, $sql_job_types);

                        // Check if there are any job types available
                        if (mysqli_num_rows($result_job_types) > 0) {
                            while ($row_job_type = mysqli_fetch_assoc($result_job_types)) {
                                ?>
                                <option value="<?php echo $row_job_type['jobtype']; ?>"><?php echo $row_job_type['jobtype']; ?></option>
                                <?php
                            }
                        } else {
                            // If no job types are available, display a message
                            echo "<option>No job types available</option>";
                        }
                        ?>
                    </select>
                </div>
                <!-- Hidden inputs for student information -->
                <?php
                $user = $_SESSION['username'];
                $sql_student = "SELECT * FROM studentlogin WHERE fname = '$user'";
                $result_student = mysqli_query($conn, $sql_student);
                if (mysqli_num_rows($result_student) > 0) {
                    while ($row_student = mysqli_fetch_assoc($result_student)) {
                        ?>
                        <input type="hidden" name="lname" value="<?php echo $row_student['lname']; ?>">
                        <input type="hidden" name="branch" value="<?php echo $row_student['branch']; ?>">
                        <input type="hidden" name="email" value="<?php echo $row_student['email']; ?>">
                        <input type="hidden" name="yop" value="<?php echo $row_student['yop']; ?>">
                        <?php  
                    }
                }
                ?>
                <div class="form-group text-center"> 
                    <button type="submit" class="btn btn-primary" style=" background: linear-gradient(to left, #4181ED, #3F4261)">Check Eligibility</button>
                </div>
            </form>
        </div>
    </div>

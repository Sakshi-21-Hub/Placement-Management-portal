...
<body>
    ...
    <div class="row">
        <div class="col-md-6 offset-md-3 mt-4">
            <form action="applyrole.php" method="POST">
                <input type="hidden" name="jobtype" value="<?php echo isset($_POST['jobtype']) ? htmlspecialchars($_POST['jobtype']) : ''; ?>">
                <div class="form-group">
                    <label for="exampleInputEmail1">Select Company</label>
                    <select class="custom-select" name="cname">
                        <?php
                        // Retrieve job type from the previous form
                        $jobtype = $_POST['jobtype'];

                        // Fetch companies associated with the selected job type from the database
                        $sql_select_company = "SELECT cname FROM job WHERE jobtype = '$jobtype' AND status='active'";
                        $result_company = mysqli_query($conn, $sql_select_company);

                        if (mysqli_num_rows($result_company) > 0) {
                            while ($row_company = mysqli_fetch_assoc($result_company)) {
                                ?>
                                <option value="<?php echo $row_company['cname']; ?>"><?php echo $row_company['cname']; ?></option>
                                <?php
                            }
                        } else {
                            echo "<option>No companies available for selected job type</option>";
                        }
                        ?>
                    </select>
                </div>

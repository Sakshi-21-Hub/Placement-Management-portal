...
<body>
    ...
    <div class="row">
        ...
        ">Select For a Company</h1>
        </div>
    </div>
    <div class="row">
        <div class="col-md-6 offset-md-3 mt-4">
            <form action="check.inc.php" method="POST">
                <input type="hidden" name="jobtype" value="<?php echo isset($_POST['jobtype']) ? htmlspecialchars($_POST['jobtype'], ENT_QUOTES, 'UTF-8') : ''; ?>">
                <input type="hidden" name="company" value="<?php echo isset($_POST['cname']) ? htmlspecialchars($_POST['cname'], ENT_QUOTES, 'UTF-8') : ''; ?>">

                <div class="form-group">
                    <label for="jobrole">Select Job Role</label>
                    <div class="col-sm-12">
                        <select class="custom-select" name="jobrole" id="jobrole">
                            <?php
                            if (isset($_POST['jobtype']) && isset($_POST['cname'])) {
                                $jobtype = $_POST['jobtype'];
                                $cname = $_POST['cname'];

                                $stmt = $conn->prepare("SELECT jobrole FROM job WHERE jobtype = ? AND cname = ? AND status = 'active'");
                                $stmt->bind_param("ss", $jobtype, $cname);
                                $stmt->execute();
                                $result_jobrole = $stmt->get_result();

                                if ($result_jobrole->num_rows > 0) {
                                    while ($row_jobrole = $result_jobrole->fetch_assoc()) {
                                        echo "<option value='" . htmlspecialchars($row_jobrole['jobrole'], ENT_QUOTES, 'UTF-8') . "'>" . htmlspecialchars($row_jobrole['jobrole'], ENT_QUOTES, 'UTF-8') . "</option>";
                                    }
                                } else {
                                    echo "<option>No job roles available for selected job type and company</option>";
                                }
                ...
                if (isset($_SESSION['username'])) {
                    ...
                    $stmt = $conn->prepare("SELECT * FROM studentlogin WHERE fname = ?");
                    ...
                    if ($result_student->num_rows > 0) {
                        ...
                        ?>
                        <input type="hidden" name="lname" value="<?php echo htmlspecialchars($row_student['lname'], ENT_QUOTES, 'UTF-8'); ?>">
                        <input type="hidden" name="branch" value="<?php echo htmlspecialchars($row_student['branch'], ENT_QUOTES, 'UTF-8'); ?>">
                        <input type="hidden" name="email" value="<?php echo htmlspecialchars($row_student['email'], ENT_QUOTES, 'UTF-8'); ?>">
                        <input type="hidden" name="yop" value="<?php echo htmlspecialchars($row_student['yop'], ENT_QUOTES, 'UTF-8'); ?>">
                        <?php
                    }
                ...
                if (isset($_POST['cname'])) {
                    ...
                    $stmt = $conn->prepare("SELECT uname FROM company WHERE companyname = ?");
                    $stmt->bind_param("s", $cname);
                    $stmt->execute();
                    $result_uname = $stmt->get_result();

                    if ($result_uname->num_rows > 0) {
                        $row_uname = $result_uname->fetch_assoc();
                        ?>
                        <input type="hidden" name="companyuser" value="<?php echo htmlspecialchars($row_uname['uname'], ENT_QUOTES, 'UTF-8'); ?>">
                        <?php
                    } else {
                        echo "<div class='alert alert-danger text-center'>No company found with the provided name.</div>";
                        exit;
                    }
                ...
                <div class="form-group row justify-content-center">
                    <div class="col-sm-6 text-center">
                        <button type="submit" name="check" class="btn btn-primary" style="background: linear-gradient(to left, #4181ED, #3F4261);">Check Eligibility</button>
                    </div>
                </div>
            </form>

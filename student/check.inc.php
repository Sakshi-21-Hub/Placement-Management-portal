...
?>  
  ...
  .img-container {
            ...
            margin-left: 20px; /* Optional: Add some margin to the right for spacing */
        }  
  .feature-box {
...
body {
        ...
        .upcoming-events-container {
            ...
            margin-top: 80px; /* Adjust as per your image's position */
            right: 10px;
            width: 380px;
            max-height: calc(100vh - 180px); /* Adjust according to your layout */
            overflow-y: auto;
...
<body>
    ...
    <div class="content mb-4">
        <h2 class="text-center">
            <u><a href="applyrole.php">Apply For a Company</a></u>
        </h2>
        <?php
        ...
        if (isset($_POST['check'])) {
            ...
            $sql_student_info = "SELECT * FROM studentlogin WHERE fname='$user'";
            ...
            if ($result_student_info && mysqli_num_rows($result_student_info) > 0) {
                ...
                ?>
                <input type="hidden" name="lname" value="<?php echo $row_student_info['lname']; ?>">
                <input type="hidden" name="fname" value="<?php echo $row_student_info['fname']; ?>">
                <input type="hidden" name="branch" value="<?php echo $row_student_info['branch']; ?>">
                <input type="hidden" name="email" value="<?php echo $row_student_info['email']; ?>">
                <input type="hidden" name="yop" value="<?php echo $row_student_info['yop']; ?>">
                <?php
                
                // Check if the student's percentage is null
                if ($row_student_info['percentage'] === null || $row_student_info['percentage'] == 0) {
                    echo "<div class='alert alert-warning text-center'>Please update your percentage in <b><u><a href='editprofile.php' class='text-danger'>Edit Profile</a></u></b> section to proceed.</div>";
                } else {
                ...
                if (mysqli_num_rows($result_company) > 0) {
                    ...
                    $compperc = isset($row['minperc']) ? $row['minperc'] : null;
                    ...
                    if (mysqli_num_rows($res1) > 0) {
                        ...
                        if ($studperc >= $compperc) {
                            if ($studperc > 80) {
                                ...
                                $message = "Good Job, Your Percentage is matching for <b>{$company} {$jobrole} {$jobtype}</b>";
                            } elseif ($studperc >= 60) {
                                $chances = 80;
                                ...
                                $message = "Not Bad, Your Percentage is quite matching for <b>{$company} {$jobrole} {$jobtype}</b>";
                            } else {
                                $chances = 50;
                                $progressBarClass = 'bg-danger';
                                $message = "Your Percentage for <b>{$company} {$jobrole} {$jobtype}</b> doesn't match expectations";
                            }
                        } else {
                            $message = "Add percentage to check eligibility <a href='editprofile.php' class='text-danger'>Edit Profile</a>";
                        }
                        ?>
                        <h5><?php echo $message; ?></h5>
                        <p class="lead">Chances of getting placed</p>
                        <div class="progress mb-4" style="width: 100%;">
                            <div class="progress-bar <?php echo $progressBarClass; ?>" role="progressbar" style="width: <?php echo $chances; ?>%;" aria-valuenow="<?php echo $chances; ?>" aria-valuemin="0" aria-valuemax="100"><?php echo $chances; ?>%</div>
                        </div>
                        <?php if ($chances >= 60) { ?>
                        ...
                        <?php } ?>
                        <p class="medium text-danger"><i class="fas fa-exclamation-circle"></i> Make sure you have updated your percentage in <b><u><a href="editprofile.php" class="text-danger">Edit Profile</a></u></b> section!</p>
                        <form action="php/apply.inc.php" method="POST">
                            <input type="hidden" name="company" value="<?php echo $company; ?>">
                            <input type="hidden" name="jobtype" value="<?php echo $jobtype; ?>">
                            <input type="hidden" name="jobrole" value="<?php echo $jobrole; ?>">
                            <input type="hidden" name="companyuser" value="<?php echo $row['uname']; ?>">
                            <div class="form-group text-center">
                                <input class="btn btn-primary" type="submit" name="check" value="Apply" <?php echo ($chances < 60) ? 'disabled' : ''; ?> style="width: 150px; color: white; font-weight: bold; background: linear-gradient(to left, #4181ED, #3F4261);">
                            </div>
                        </form>
                        <?php
                    } else {
                        echo "<div class='alert alert-danger text-center'>Student information not found.</div>";
                    }
                } else {
                    echo "<div class='alert alert-danger text-center'>Company information not found.</div>";
                }
            } 
        }
    }

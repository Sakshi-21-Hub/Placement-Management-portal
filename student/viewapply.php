...
<body>
    ...
    <div class="row">
        ...
        ">UMIT Placement Management Portal - Apply Now</h1>
            <div class="mb-5">
                <a href="applytype.php" class="btn btn-primary">Apply Now</a>
            </div>
            <?php
            ...
            AND status!='Rejected' 
            AND status!='Unknown' 
            AND companyname IN (SELECT companyname FROM company) 
            AND jobrole IN (SELECT jobrole FROM company) 
            ORDER BY id DESC;";
            $sql1 ="SELECT * FROM `applied` 
            WHERE name='$user' 
            AND status!='Selected' 
            AND status!='Rejected' 
            AND cname IN (SELECT cname FROM job) 
            AND jobtype IN (SELECT jobtype FROM job) 
            ORDER BY id DESC;";
          ...
          if (mysqli_num_rows($res) > 0) {
                      ...
                      <ul class="list-unstyled">
                          <li class="lead mb-3">
                              <div class="alert alert-dark" role="alert">
                                  <a href="viewcompanies.php?id=<?php echo $row['id']; ?>" class="text-dark" style="text-decoration: none;">
                                      <div>
                                          <h4><?php echo 'Company Name: ' . $row['companyname']; ?></h4>
                                          <h4><?php echo 'Job Type: ' . $row['jobtype']; ?></h4>
                                          <h4><?php echo 'Job Role: ' . $row['jobrole']; ?></h4>
                                      </div>
                                  </a>
                              </div>
                          </li>
              ...
              ?>
              <div class="lead alert alert-danger" role="alert">
                  You haven't applied for any companies!
              </div>
              <?php
          }
          ?>
